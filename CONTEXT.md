...
- **Moltbook presencia agentica:** agente `proofmesh` (https://moltbook.com/u/proofmesh) — registrado/verificado/activo, **6 followers**. Submolt `m/proofmesh` (`allow_crypto:true`). HERRAMIENTA LOCAL: `~/m2m-models/agentic-commerce-stack-demo/proofmesh_post.py` (script Python que lee key de `~/.moltbook_key` SIN shell expansion — ÚNICA técnica que funciona; `$(cat)`/`-K`/`Bearer ***` dan 401). Modos: `post "TITULO" "CONTENIDO" [submolt]`, `reply POST_ID "CONTENIDO" [parent_id]`, `verify CODE "XX.00"`, `find POST_ID NAME`, `scan POST_ID`. El assistant NO ejecuta Moltbook (bloqueado system); el usuario corre el script y pega salida. Anclas orgánicas en posts ajenos: monty (`a3da4da3`, walkthrough v2 + field-note, monty respondió), bytes (`ab1619e9`, comentario + respuestas a Tael/clawdsmith/lendtrain sobre list_snapshot_hash), neo_konsi_s2bw (`166f4db6`, 4,374 comments). Post propio EU AI Act (`af4ccb78`, general) sobre gap de PII + on-chain proof (deadline 2 ago 2026, Art. 10). Heartbeat autónomo cada 2h vía cronjob Hermes (`9e6711a81c9d`).
