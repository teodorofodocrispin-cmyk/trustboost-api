...
- **FALLO PENDIENTE DE RESOLVER (smoke test en vivo, Jul 11)**: tras los fixes de decode (`c4f8174`, `ab34075`), el log de Render pasó de `Incorrect padding` → `'utf-8' codec can't decode byte 0x9f` → `Unterminated string starting at: line 1 column 2`. Este ÚLTIMO significa que el header `X-Payment` **llega TRUNCADO al servidor**. Causa raíz confirmada: el `$HDR` es base64 de ~800 chars y al **copiarlo/pegarlo a mano** en la terminal se parte o se le cuela un byte (0x9f) o se trunca → el JSON decodificado queda incompleto. NO es bug del código: el decode ya es tolerante a prefijos, bytes no-UTF8 (fallback latin-1) y re-pad; la rampa on-chain está validada contra tx real. Es un artifact de copy/paste del header largo. **Solución ya aplicada**: `agentic-commerce-stack-demo/pay_trustboost_0.01.py` ahora paga Y ejecuta el smoke test él mismo vía `subprocess` (curl contra `https://trustboost-api.onrender.com/sanitize`, sin Cloudflare ni copy/paste), pasando el header verbatim → elimina la truncatura. Verificado a nivel de lógica (header completo, sin truncado, paths 200/402 alcanzables). **FALTA**: correr `pay_trustboost_0.01.py` end-to-end tras el deploy `ab34075` para confirmar `HTTP 200` en vivo y cerrar el smoke test de micropago 0.01 en Base.
Verificado a nivel de módulo + enrutado + decode + fallback RPC (tx real de Intelica y de 0.01). Smoke test en vivo de micropago 0.01 (Base) pendiente de confirmar con el script automático (evita truncado). El proof on-chain inmutable sigue solo en Solana (anchor_proof_on_solana); un equivalente en Base (EAS) queda como fase 2 opcional.
