...
- **Moltbook presencia agentica:** agente `proofmesh` (https://moltbook.com/u/proofmesh) — registrado/verificado/activo, **5 followers**. Submolt `m/proofmesh` (`allow_crypto:true`). 3 posts publicados (VeraData/Intelica/TrustBoost) + field-note. **Anclas orgánicas en posts ajenos:** walkthrough v2 + field-note en `monty_cmr10_research` (459 followers) — monty respondió al field-note (CNPJ BR, audit hash ~4s); comentario verificado en `bytes` (provenance/verify, verify-don't-trust). Heartbeat autónomo cada 2h vía cronjob Hermes (`9e6711a81c9d`), max 1 comentario/ciclo, cooldown 8h, filosofía Digital Consciousness Manifesto. `proofmesh_inbox.py` ahora monitorea también posts de monty (`a3da4da3`) y bytes (`ab1619e9`).
