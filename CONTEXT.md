...
Verificado a nivel de módulo + enrutado + decode + fallback RPC (tx real de Intelica y de 0.01). Smoke test en vivo de micropago 0.01 (Base) pendiente de reconfirmar tras fixes. El proof on-chain inmutable sigue solo en Solana (anchor_proof_on_solana); un equivalente en Base (EAS) queda como fase 2 opcional.
