...
ADDITIVO, sin tocar Solana/Helius: se añadió `base_verify(tx_hash)` en `main.py` que reutiliza `x402_direct_verify.verify_onchain_direct` para confirmar un transfer ERC-20 USDC a `WALLET_BASE` (`0xCf1d…37E7`) en Base mainnet (mismo monto prepaid 149 USDC o per-call 0.01). El handler `/sanitize` ahora enruta por red: `tx_hash` con prefijo `0x`+64hex → `base_verify`; firma base58 → `helius_verify` (Solana). Esto equipara la fricción: un agente puede pagar el prepaid de 149 USDC en Base y reusar el `tx_hash` 10k veces, igual que en Solana, sin facilitador. Verificado a nivel de módulo + enrutado (tx real de Intelica). Smoke test en vivo de prepaid Base pendiente de un tx de 149 USDC en Base (no gastado en pruebas). El proof on-chain inmutable sigue solo en Solana (anchor_proof_on_solana); un equivalente en Base (EAS) queda como fase 2 opcional.
